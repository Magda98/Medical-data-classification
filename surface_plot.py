...
if __name__ == "__main__":
    ...
    if order == 1:
        ...
        Z = np.dot(np.c_[np.ones(X.shape), X, Y, X*Y,
                         X**2, Y**2], C).reshape(data_1.shape)
    elif order == 3:
        Z = polyval2d(data_1, data_2, polyfit2d(
            data_1.flatten(), data_2.flatten(), data_3))
    fig = plt.figure()
    ...
    # srednia SMA
    loss = []
    with open('cnn_loss_walr.csv') as f:
        for line in f:
            x = [float(s) for s in line.rstrip('\n').split(';')]
            loss.append(x[0])
    ma_loss = []
    loss = np.asarray(loss)
    period = 0
    for index, x in enumerate(loss):
        ma_loss.append(statistics.mean(loss[period:index+1]))
        if period + 30 < index:
            period += 1
    y_t = np.arange(round(min(ma_loss), 0)+2, round(max(ma_loss), 0), step=2)
    ...
    with open('pk_heart-disease-clevlend.csv') as f:
        for line in f:
            ...
            test_2.append(x[0])
    ax = plt.figure().gca()
    x = ['1 z n,\n funkcja aktywacji softmax', 'rzeczywistoliczbowy,\n neuron o liniowej \nfunkcji aktywacji']
    ...
    plt.bar(x_pos, energy, color='teal', yerr=std,
            capsize=10, alpha=0.5, width=0.4)
    plt.xlabel("sposób kodowania wyjścia")
